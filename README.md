...
- **FTA** 利用多轮全局/本地模型上的 loss、confidence 和 rescaled logit 轨迹斜率区分成员与非成员，既利用训练过程中的持续泄漏，又避免单样本梯度计算和额外攻击模型训练，适合低成本审计。
- **KTA** 通过知识迁移训练 IN/OUT 参考模型，使参考模型更接近目标模型行为，在计算资源充足时进一步提高审计能力。
- **尚未覆盖**：论文主要研究样本级成员隐私，未系统处理数据重构、属性推断、标签分布泄漏，也未重点验证动态客户端、缺失轮次和非平稳数据；因此其结论不能直接代表这些风险或真实动态部署下的效果。
- **当前不足及后果**：主要面向半诚实被动攻击者，面对恶意服务器、恶意客户端或主动攻击时可能低估实际风险；FTA依赖可靠的 non-member 数据校准阈值，校准数据不足或分布失配会增加误报、漏报；训练轨迹剧烈波动时 Slope 分布更难分离，会降低审计准确性；部分低泄漏任务中的 Slope 接近随机，难以提供可靠判断；KTA需要训练多个参考模型，计算开销较大，不利于轻量、实时和大规模部署。

### 6.2 对群智感知感知层 FL 的帮助

群智感知中的移动用户、传感设备和边缘节点通常具有多轮训练、数据高度异构、数据量不均及动态参与等特点。本文可提供一种“基于训练轨迹持续评估隐私风险”的思路，用于比较不同感知客户端的成员泄漏、分析模型收敛后风险是否仍在累积，并为后续保护和调度提供量化依据；它是隐私审计方法，而非可直接替代现有群智感知算法的完整方案。

### 6.3 小论文中可直接借鉴的方法

- 记录各通信轮次的 loss、confidence 或 rescaled logit，计算样本的 Slope Signal，并以 AUC、TPR@低FPR 评价成员隐私风险。
- 在感知层 FL 中加入轻量 FTA 审计模块，分别统计客户端风险及其随轮次变化的趋势。
- 沿用论文的变量设计，对客户端数量、本地数据量、IID/Non-IID 程度、通信轮数、模型结构和参与率开展对照或消融实验。

### 6.4 可考虑的创新点

1. **动态客户端与缺失轮次下的鲁棒 Slope 审计**：仅利用实际参与轮次，并研究不规则、稀疏轨迹上的可靠估计。
2. **动态 FL 的滑动窗口/分段 Slope**：突出近期变化，减少完整历史平均造成的风险发现滞后。
3. **隐私风险驱动的客户端选择与参与控制**：依据审计结果调整参与频率、聚合权重或保护强度，形成“检测—干预”闭环。
4. **多信号融合审计**：自适应融合 loss、confidence 与 logit 的斜率，提升跨模型、跨感知任务的稳定性。
5. **从样本级扩展到用户/客户端级成员隐私**：聚合同一参与者的多条感知记录，并重新定义成员标签与用户级评估指标。

> 不将“Non-IID + 客户端专属阈值”单列为主要创新：原 FTA 已可由各客户端使用自己的 non-member 数据校准阈值；更值得研究的是校准数据稀缺、分布漂移或验证集不可靠时如何保持阈值稳定。
