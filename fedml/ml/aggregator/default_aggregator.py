...
class DefaultServerAggregator(ServerAggregator):
    ...
    def test(self, test_data, device, args, is_test=True):
        # test data
        test_num_samples = []
        test_tot_corrects = []
        test_losses = []
        ...
        print(sum(test_num_samples))
        test_acc = sum(test_tot_corrects) / sum(test_num_samples)
        test_loss = sum(test_losses) / sum(test_num_samples)
        if self.args.enable_wandb:
            wandb.log({f"{prefix}/Acc": test_acc, "round": args.round_idx})
            wandb.log({f"{prefix}/Loss": test_loss, "round": args.round_idx})

        mlops.log({f"{prefix}/Acc": test_acc, "round": args.round_idx})
        mlops.log({f"{prefix}/Loss": test_loss, "round": args.round_idx})
